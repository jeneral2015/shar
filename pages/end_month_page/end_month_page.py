...
class EndMonthPage:
    ...
    def distribute_miscellaneous(self, e):
        from pages.end_month_page.distribute_miscellaneous import DistributeMiscellaneous  # تغيير هنا
        DistributeMiscellaneous(self.page, self.db).show_confirmation()

    def clear_all_data(self, e):
        from pages.end_month_page.clear_data import ClearData  # تغيير هنا
        ClearData(self.page, self.db).show_confirmation()

    def end_month_process(self, e):
        from pages.end_month_page.finalize_month import FinalizeMonth  # تغيير هنا
        FinalizeMonth(self.page, self.db, self.navigate).start_process()
